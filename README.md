...
- **手动调度面板**：`az.严管调度.*` 默认优选家宽出口，`az.其他调度.*` 默认优选美国分区组，不包含订阅原始总选择组。
- **默认代理自动识别**：按 `PROXY`、`节点选择`、`手动选择`、`GLOBAL` 查找订阅默认代理组，并忽略这些核心词前后的图标或说明。
...
脚本会把生成的分区测速组追加到订阅中实际存在的常见默认代理组；匹配时只看 `PROXY`、`节点选择`、`手动选择`、`GLOBAL` 四个核心词，忽略前后缀。没有匹配到常见默认组时，DoH 端点规则会回落到 `az.核心出口.🏠 家宽出口`，避免写入不存在的目标。
...
测试通过 `vm` 加载单文件覆写脚本，覆盖 16 个纯函数单元测试和 21 个端到端集成测试。
