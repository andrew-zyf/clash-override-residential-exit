...
使用 `vm` 隔离加载配置脚本和实现脚本，覆盖 `dns-sniffer-only`、`merged`、凭证校验、缺失配置、地区 fallback、开关组合、缺失地区报错、幂等重跑、受管对象修复、拆分文件传参等 18 个用例。
...
| `tests/validate.js` | 测试 | `vm` 隔离运行 18 个用例 |
...
- **代理类型白名单**：`BASE` 新增 `validProxyTypes` 常量，覆盖 Clash 支持的 12 种代理类型。`buildMiyaProxy` 在生成 MiyaIP 节点前校验硬编码的 `"http"` 是否在白名单内，防止常量被误改后静默生成非法类型。
- 新增 3 个单元测试：port 边界校验、`validProxyTypes` 常量完整性、`buildMiyaProxy` 异常路径。

### v11.5 — 2026-02

- 首次公开版本。统一策略引擎（POLICY）、DNS/Sniffer 三阶防泄漏、链式代理与媒体分离。
