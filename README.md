...
- **完整链式代理怎么配？** 使用 `overrideMode: "merged"`，并在配置文件里填入 `MIYA_CREDENTIALS`。实现脚本会先完成凭证和 `chainRegion` 前置校验，再接管 DNS / Sniffer、注入链式代理和媒体分流规则。
- **链式代理里 `自选节点 => 家宽IP` 显示 timeout 是什么原因？** 这个节点是家宽出口 HTTP 代理，脚本会给它写入 `dialer-proxy`，让它经当前 `chainRegion` 的分区测速组出站。因此 Clash 对它测速时测的是「自选节点 → 家宽 IP → 探测 URL」整条链路；如果家宽端点、凭证、或自选节点到家宽端点的链路不可达/过慢，就会显示 timeout。
- **`merged` 报错 `请先填写 MIYA_CREDENTIALS...`** — 凭证仍是空值，请确认已在 `MIYA_CREDENTIALS` 中填入用户名、密码、家宽出口和官方中转端点。
...
使用 `vm` 隔离加载合并脚本，覆盖 12 个纯函数单元测试 + 20 个端到端集成测试（`dns-sniffer-only` / `merged` / 凭证校验 / 地区 fallback / 地区名识别 / 开关组合 / 幂等重跑 / 受管对象修复 / 前置校验等）。
...
| `tests/test.js` | 测试 | `vm` 隔离运行 12 个纯函数 + 20 个端到端用例 |
