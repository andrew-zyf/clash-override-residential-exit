...
- **删除链式 relay 逻辑**：不再生成 `自选节点 => 家宽IP`，不再写 `dialer-proxy`，也不再读取 `chainRegion`。家宽核心组只保留 `MiyaIP（官方中转）`，升级时会清理旧 relay 节点、旧核心组和选择组里的旧引用。
- **核心组改名**：`az.核心链路.🔗 链式代理-家宽出口` 改为 `az.核心链路.🔗 家宽出口`。
- **调度组改为纯手动出口面板**：所有 `az.严管调度.*` 与 `az.其他调度.*` 的候选项统一为 `az.核心链路.🔗 家宽出口` + 已存在的 `az.分区测速.*节点组`。
- **移除调度组里的 `办公娱乐好帮手`**：避免调度面板再兜底到订阅原始自动选择组。家宽链路断开时，可在对应 `az.*调度.*` 面板手动切分区测速组直出。
- **凭证简化**：`MIYA_CREDENTIALS` 只需要用户名、密码和官方中转端点；旧 `relay` 字段会被忽略。
